...
public class Dialogue : MonoBehaviour
{
   /* [NonSerialized]*/ public DialogueSystem dialogueSystem;
    ...
    public List<EachDialogue> thisIdDialogues; // 지정한 대화ID가 일치하는 대사들만 모아놓는다. 이것을 DialogueSystem에 전달.
    ...
    void Start()
    {
        ...
        dialogueJson = Resources.Load<TextAsset>("Dialogue");
    ...
    void FindNextDialogue()
    {
        ...
        FindDialogueByID(0);
        thisIdDialogues = dialogueContainer.dialogueList;
    ...
    void FindDialogueByID(int targetDiaID)
    {
        ...
        for (int i = 0; i < dialogueContainer.dialogueList.Count; i++)
        {
            if (dialogueContainer.dialogueList[i].diaID == targetDiaID)
            {
                ...
                sentenceText.text = dialogueContainer.dialogueList[i].dialogueText;
                dialogueFound = true;
